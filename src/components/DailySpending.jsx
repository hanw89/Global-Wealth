          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {spendingData
                ...
                .map((item) => (
                <div key={item.id} className="p-5 rounded-2xl border border-slate-100 bg-slate-50/50 hover:bg-white hover:shadow-md transition-all group">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">Budget Verified</span>
                  </div>
                  ...
                  <div className="mt-4 w-full bg-slate-200 h-1.5 rounded-full overflow-hidden">
                    <div 
                      ...
                      style={{ width: `${(item.amount / totalSpending * 100).toFixed(0)}%` }}
                    ></div>
                  </div>
                  <p className="mt-2 text-[10px] text-slate-400 font-medium">
                    {(item.amount / totalSpending * 100).toFixed(1)}% of total monthly spending
                  </p>
                </div>
              ))}
            </div>
            ...
            {selectedCategory === 'All' && (
              <div className="mt-8 pt-6 border-t border-slate-100">
                ...
                <div className="w-full bg-slate-100 h-3 rounded-full overflow-hidden">
                  <div className="bg-slate-900 h-full w-[78%] rounded-full"></div>
                </div>
              </div>
            )}
          </div>
